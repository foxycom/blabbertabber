...
JAVA_OPTS=" -Dabc.instrument.array.operations -Dabc.instrument.fields.operations -Dabc.taint.android.intents -Dabc.instrument.include=com.blabbertabber.blabbertabber  -Dabc.instrument.multithreaded "

INSTRUMENTATION_OPTS=" \
--skip-class=com.blabbertabber.blabbertabber.R \
...
if [ "$$IS_RUNNING" == "0" ]; then \
        ...
        APP_INSTALLED=$$($(ADB) shell pm list packages | grep -c "com.blabbertabber.blabbertabber$$"); \
        INSTRUMENTATION_TESTS_INSTALLED=$$($(ADB) shell pm list packages | grep -c "com.blabbertabber.blabbertabber.test$$"); \
        if [ "$$APP_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing app"); \
                $(ADB) uninstall com.blabbertabber.blabbertabber > /dev/null; \
        else \
                (>&2 echo "App not installed"); \
        fi; \
        if [ "$$INSTRUMENTATION_TESTS_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing test app"); \
                $(ADB) uninstall com.blabbertabber.blabbertabber.test > /dev/null; \
        else \
...
$(shell if [ "$(TRACING_APKS_INSTALLED)" == "0" ]; then \
	...
	$(ADB) shell pm clear com.blabbertabber.blabbertabber > /dev/null; \
fi)
...
#	Log directly to the expected file
	$(ADB) shell am instrument -w -e class $(TEST_NAME) com.blabbertabber.blabbertabber.test/android.support.test.runner.AndroidJUnitRunner 2>&1 | tee $(@)
#	Copy the traces if the previous command succeded
	@export ABC_CONFIG=$(ABC_CFG) && $(ABC) copy-traces com.blabbertabber.blabbertabber ./traces/$(TEST_NAME) force-clean
