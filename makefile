...
JAVA_OPTS=" -Dabc.instrument.fields.operations -Dabc.taint.android.intents -Dabc.instrument.include=com.blabbertabber.blabbertabber"
...
espresso-tests.log : app-original.apk app-androidTest.apk running-emulator
	...
	$(ADB) shell am instrument -w -r com.blabbertabber.blabbertabber.test/android.support.test.runner.AndroidJUnitRunner | tee espresso-tests.log
	export ABC_CONFIG=$(ABC_CFG) && $(ABC) stop-all-emulators
	$(RM) running-emulator
...
%.testlog: app-androidTest.apk app-instrumented.apk running-emulator
	...
	echo $(ADB)
	$(eval FIRST_RUN := $(shell $(ADB) shell pm list packages | grep -c com.blabbertabber.blabbertabber))
	echo $(FIRST_RUN)

	@if [ "$(FIRST_RUN)" == "2" ]; then \
		echo "Resetting the data of the apk"; \
		$(ADB) shell pm clear com.blabbertabber.blabbertabber; \
	else \
	...
	$(ADB) shell am instrument -w -e class $(TEST_NAME) com.blabbertabber.blabbertabber.test/android.support.test.runner.AndroidJUnitRunner | tee $(@)
...
copy-shadows :
	cp -v ./shadows/*.java app/src/carvedTest/com/blabbertabber/blabbertabber
